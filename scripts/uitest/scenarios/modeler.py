...
def run(session):
    ...
    check("L2", "modeler bundle stays off other pages",
    ...
    check("B4", "deploy button reported success",
    ...
    check("B5", "deployment listed without pressing Refresh",
          deployment_name in table.inner_text(), table.inner_text()[:200])
    ...
    shot(session, page, "controlpanel-deployments", "B5",
         mask=[page.locator("#deployments-table tbody td:nth-child(1)"),
    ...
    page.wait_for_timeout(2500)
    check("B6", "deployment deleted",
          deployment_name not in page.locator(
              "#deployments-table tbody").inner_text())
    ...
    direct = rest(page, base, "/@bpmproxy-deploy", "POST",
                  {"name": f"{PROCESS_ID}.bpmn", "xml": PROBE_BPMN})
    check("B4", "deploy service accepts a POST",
          direct["status"] == 200, f"{direct['status']} {direct['body'][:160]}")
    ...
    check("B4", "deploy service is not routed without an Accept header",
          no_accept == 404, str(no_accept))

    # B7 -- a Site Administrator outside camunda-admin gets nothing.
    editor = session.context("editor")
    response = editor.request.get(f"{base}/@bpmproxy-deployments",
                                  headers={"Accept": "application/json"})
    check("B7", "deployments endpoint refused to a Site Administrator",
          response.status in (401, 403), str(response.status))
