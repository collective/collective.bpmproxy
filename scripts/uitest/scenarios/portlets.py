...
def run(session):
    ...
    if assigned["status"] != 200:
        check("F0", "renovation-project-demo with its portlets is available",
              False, str(assigned["status"]))
        return
    ...
    check("F1", "submit-plan starts the Plan Review process",
          result["status"] == 200, f"{result['status']} {result['body'][:200]}")
    ...
    check("F2", "task list portlet rendered on the project",
    ...
    check("F5", "message portlet rendered on the project",
          "Request extra work" in column_text
          or "Request extra work" in page.content(),
          column_text[:200])
    ...
    for index in range(button.count()):
        ...
        if "extra work" in label.lower():
            ...
            break
    if fired:
        check("F5", "dispatching the message does not error",
              "Internal Server Error" not in page.content(), page.url)
    else:
        check("F5", "message button found to click", False,
    ...
    check("F4", "redirect view handles an unknown task id gracefully",
          "Internal Server Error" not in page.content(), page.url)
