...
def run(session):
    ...
    body = page.content()
    check("C7", "submit reported success", "Submit successful" in body, url)
    check("C7", "redirected into a task form", "/@@bpm-task/" in url, url)
    ...
    crumbs = page.locator(
        "#portal-breadcrumbs, .breadcrumb, nav[aria-label='breadcrumb']"
    )
    crumb_text = crumbs.first.inner_text() if crumbs.count() else ""
    check(
        "D2",
        "breadcrumbs include the task title",
        bool(crumb_text.strip()),
        crumb_text[:160],
    )
    ...
    check(
        "C9",
        "task list shows the running instance's task",
    ...
    check(
        "D4",
        "unknown task is refused with a message",
    ...
    check(
        "C7",
        "the proxy still renders after the round trip",
