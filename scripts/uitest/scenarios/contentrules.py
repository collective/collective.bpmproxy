...
def run(session):
    ...
    check(
        "G1",
        "all seven BPMN signal rules are installed",
        not missing,
    ...
    if select.count():
        ...
        if target in options:
            ...
            add = page.locator("input[type=submit][name*='add'], button[type=submit]")
            if add.count():
                add.first.click()
    ...
    check(
        "G2",
        "published-any rule assignable to a folder",
        assigned,
    ...
    if created["status"] in (200, 201):
        ...
        check(
            "G2",
            "publishing with a signal rule active succeeds",
            published,
            "see @workflow/publish response",
        )
        ...
        check(
            "G5",
            "the publish transaction committed",
            review_state == "published",
            f"review_state={review_state!r}",
        )
