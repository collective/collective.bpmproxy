...
def run(session):
    ...
    check(
        "A2",
        "collective.bpmproxy reports itself installed",
    ...
    check(
        "A3",
        "collective.bpmproxy.modeler installed",
    ...
    check(
        "A3",
        "control panel listed in Site Setup",
    ...
    check(
        "A5", "camunda-admin group exists", group["status"] == 200, str(group["status"])
    )
    members = (group["json"] or {}).get("users", {})
    member_ids = (
        {u.get("id") for u in (members.get("items") or [])}
        if isinstance(members, dict)
        else set()
    )
    check(
        "A5",
        "camunda-admin contains Administrators",
    ...
    record = rest(page, base, "/@registry/collective.bpmproxy.tenant_ids")
    check(
        "A7",
        "tenant_ids registry record present",
        record["status"] == 200,
        f"{record['status']} {record['body'][:120]}",
    )
