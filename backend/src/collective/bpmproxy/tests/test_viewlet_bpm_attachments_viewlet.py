...
class ViewletIntegrationTest(unittest.TestCase):
    ...
    def test_bpm_attachments_viewlet_is_registered(self):
        # The viewlet is registered for the BpmProxyTaskFormView on the
        # plone.belowcontentbody manager (see viewlets/configure.zcml).
        from collective.bpmproxy.views.bpm_form_view import BpmProxyTaskFormView

        view = BpmProxyTaskFormView(self.portal["other-document"], self.request)
        ...
        manager.update()
        my_viewlet = [
            v for v in manager.viewlets if v.__name__ == "bpm-attachments-viewlet"
        ]  # NOQA: E501
