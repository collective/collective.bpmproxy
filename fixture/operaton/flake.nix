{
  description = "Operaton Application";
  ...
  inputs = {
    ...
    # Flakes
    mvn2nix = { url = "gitlab:vasara-bpm/mvn2nix"; inputs.nixpkgs.follows = "nixpkgs"; inputs.utils.follows = "flake-utils"; };
  };
...
#!/usr/bin/env bash
exec ${pkgs.temurin-jre-bin-21}/bin/java ''\\$@ -jar $out/var/lib/${call-name}.jar
EOF
    ...
    packages.jar = let mavenRepository = pkgs.buildMavenRepositoryFromLockFile {
      ...
      buildInputs = with pkgs; [ jdk21_headless maven ];
      buildPhase = ''
    ...
    devShells.default = pkgs.mkShell {
      buildInputs = [
        ...
        pkgs.poetry
        pkgs.maven
        (inputs.mvn2nix.defaultPackage.${system}.override { jdk = pkgs.jdk21; })
      ];
      shellHook = ''
      '';
    };
