...
### Current Status: Migrations Landed

Four major changes are complete and committed (`5bd0416 feat: Migrate to
Operaton`, `67f6a71 feat: Upgrade Operaton cockpit plugins`):
...
```
collective.bpmproxy/                      # monorepo root -- NO Python package here
├── Makefile                              # Delegates to backend/; frontend build; devenv; e2e
├── README.md                             # Monorepo orientation
├── AGENTS.md                             # This file
├── devenv.nix / devenv.yaml / devenv.lock  # devenv.sh: postgres, keycloak, mailpit, operaton
├── devenv/keycloak/realm-plone.json      # Keycloak dev realm (client "operaton", group camunda-admin)
├── .pre-commit-config.yaml               # Ruff + hygiene hooks
├── .github/workflows/plone-package.yml   # CI: lint, test, build, frontend, operaton-fixture
├── ec-ed25519-{priv,pub}-key.pem         # Generated by `devenv shell`; untracked (.gitignore *.pem)
│
├── backend/                              # <-- the Plone add-on lives here
│   ├── pyproject.toml                    # PEP 621 (hatchling), pytest/ruff/towncrier, uv constraints
│   ├── uv.lock                           # uv lockfile (`make lock` regenerates)
│   ├── Makefile                          # install/lock/test/test-offline/test-live/lint/format/i18n/start
│   ├── tox.ini                           # Legacy pip matrix; not referenced by CI or any Makefile
│   ├── README.md                         # Comprehensive user documentation
│   ├── CHANGES.rst / CONTRIBUTORS.rst / DEVELOP.rst
│   ├── LICENSE.GPL / LICENSE.rst         # GPLv2
│   ├── news/                             # towncrier fragments
│   ├── docs/                             # Sphinx skeleton (conf.py, index.rst) -- currently a stub
│   ├── scripts/                          # update_plone_constraints.py, configure_blobstorage.py
│   ├── instance/                         # Generated Zope instance (untracked)
│   └── src/collective/bpmproxy/          # Main Python package
│       ├── client.py                     # Operaton REST facade; EdDSA JWT minting
│       ├── utils.py                      # Variable inference, form prep/validation, SideEffectDataManager
│       ├── interfaces.py                 # Constants + browser layer
│       ├── setuphandlers.py              # Post-install; HiddenProfiles
│       ├── upgrades.py                   # Registry cleanup upgrade steps
│       ├── testing.py                    # Plone test layers
│       ├── renovation_demo.py            # renovation_demo profile post_handler
│       ├── behaviors/process_context.py  # IProcessContext marker + IProcessContextBehavior schema
│       ├── content/                      # IBpmProxy, IBpmAttachments, IBpmAttachment
│       ├── views/                        # Start/task form views, attachments, orphans
│       ├── services/                     # @bpmproxy-deploy, @bpmproxy-deployments
│       ├── portlets/                     # Tasks + Signal portlets
│       ├── actions/                      # BpmSignal content-rule action
│       ├── adapters/                     # Local roles, substitutions, patterns
│       ├── subscribers/                  # Auto-complete tasks on add/edit
│       ├── viewlets/                     # Attachment + related-task viewlets
│       ├── vocabularies/                 # VocabItem + process/attachment vocabularies
│       ├── modeler/                      # Optional modeler add-on (profiles, upgrades)
│       ├── browser/                      # Browser layer, control panel, jbot overrides
│       │   └── static/                   # Committed build output of frontend-classic/
│       ├── profiles/                     # default/, uninstall/, renovation_demo/
│       ├── locales/                      # i18n catalogs + update_locale script
│       └── tests/                        # 38 test modules + robot/ suites
│
├── frontend-classic/                     # Vite sources for the Classic UI bundles
│   ├── package.json / package-lock.json
│   ├── tsconfig.json                     # Type-check config (`npm run typecheck`)
│   ├── build.mjs                         # Three IIFE library builds -> backend browser/static/
│   └── src/                              # common, form, diagram, controlpanel, modeler, dmn, form-playground
│
├── frontend/                             # Placeholder for a future Volto add-on (README only)
│
├── fixture/operaton/                     # Custom Operaton app (Spring Boot 4 / Java 21)
│   ├── pom.xml                           # Operaton BOM 2.1.0; artifact plone-operaton-app
│   ├── Makefile                          # build/test/start/dist/push/fetch-plugins
│   ├── flake.nix / mvn2nix-lock.json     # Reproducible Nix packaging
│   ├── mvnw / .mvn/wrapper/              # Maven wrapper (devenv runs ./mvnw)
│   └── src/main/
│       ├── java/com/example/camunda/     # Package not yet renamed post-Operaton
│       │   ├── Application.java           # @SpringBootApplication @EnableProcessApplication
│       │   ├── ProcessEngineConfig.java   # Engine plugins; registers the JWT filter
│       │   ├── Engine.java                # Auto-migration + default authorizations
│       │   ├── Engine*Listener.java       # Parse/decision/business-key listeners
│       │   ├── JWTTokens.java             # Shared JWT verification (signature + alg + exp/nbf)
│       │   ├── JWTAuthenticationProvider.java  # Bearer-token auth for /engine-rest/*
│       │   └── JWTIdentityService.java    # Maps verified claims -> Operaton identity
│       └── resources/
│           ├── application.yml            # Default (H2) + `postgres` + `oauth2` profiles
│           ├── app/...                    # Cockpit plugin JS (duplicate of the webjars copy)
...
|---|---|---|
| `operaton` | `8081` | The Spring Boot app from `fixture/operaton/`, run via `./mvnw spring-boot:run` with `SPRING_PROFILES_ACTIVE=postgres,oauth2` and `PLONE_PUBLIC_KEY` pointing at the repo-root ed25519 public key. Waits for Keycloak to serve the realm before starting (issuer metadata is resolved at boot). |
| `keycloak` | `8082` | OIDC provider for the Operaton webapps. Imports realm `plone` from `devenv/keycloak/realm-plone.json` on startup. Admin console: `admin`/`admin` (master realm). |
...
`devenv up` is wrapped by `make services` at the repo root. `devenv shell` additionally provides JDK 21 + Maven + Python, generates the ed25519 keypair if missing (`enterShell`), and exports `CAMUNDA_API_URL` / `CAMUNDA_API_PRIVATE_KEY` / `PLONE_PUBLIC_KEY` / `KEYCLOAK_ISSUER_URI`. Plone itself is started manually (`make start`, port 8080), which builds `backend/.venv` on first run.
...
**Keycloak dev realm** (`devenv/keycloak/realm-plone.json`): confidential client `operaton` (secret `operaton-secret`, redirect URIs `http://localhost:8081/*`, post-logout redirect configured) with a client-level `oidc-group-membership-mapper` emitting group names into a `groups` claim (`full.path=false`); group `camunda-admin`; users `admin`/`admin` (member of `camunda-admin`) and `user`/`user` (no groups → welcome app only). Also a second, service-account-only client `operaton-worker` (secret `operaton-worker-secret`, `client_credentials` grant) for external task workers talking to `engine-rest` directly — its token hardcodes an `operaton_username: admin` claim plus the same `groups` mapper, so it authenticates as the persistent `admin` user (see `examples/renovation-bot/` and `examples/renovation-bot-py/`).

To persist realm edits made through the Keycloak admin console back into `realm-plone.json`, run `keycloak-realm-export plone devenv/keycloak/realm-plone.json` from `devenv shell` after stopping the `keycloak` process (`devenv processes down keycloak` or `Ctrl-C` on `devenv up`) — the exporter needs exclusive access to Keycloak's embedded dev-file database. This script comes from the devenv `services.keycloak` module (`services.keycloak.scripts.exportRealm`, on by default) and is independent of any realm's `export` flag; `realms.plone.export` is deliberately left unset here, since setting it `true` only adds a `keycloak-realm-export-all` *process* that is gated on the `keycloak` process exiting — which never happens under `devenv up` — so it would just sit "waiting" in `devenv up`'s process list forever.
...
**Security:**
- `JWTAuthenticationProvider.java` — Guards `/engine-rest/*` (`jakarta.servlet`). Plone's own `Bearer <token>` (ed25519, BouncyCastle EdDSA, verified via `JWTIdentityService`) is always tried first, in every configuration. As a fallback it accepts *either* Keycloak-issued JWTs *or* HTTP Basic — never both — chosen by `JWTIdentityService.isOAuth2Enabled()` (same flag as the webapp OIDC login below); an unrecognized or absent credential is rejected (401) either way.
- `JWTIdentityService.java` — Extracts `sub`, `groups`, `tenant_ids` from Plone's JWT claims and sets Operaton's transient authentication. Also verifies Keycloak JWTs against the realm's JWKS (issuer + signature + expiry; audience not enforced) and resolves them to a **persistent** Operaton user: the `operaton_username` hardcoded claim if present (e.g. the `operaton-worker` service-account client), else `preferred_username`; group membership from the token's `groups` claim, falling back to the resolved user's persistent group membership.
- `operaton-bpm-spring-boot-starter-security` — under the `oauth2` Spring profile, OIDC login for the webapps (client registration `keycloak`, `operaton.bpm.oauth2.identity-provider` with `group-name-attribute: groups`, SSO logout). Without the profile the starter's `OperatonBpmSpringSecurityDisableAutoConfiguration` installs a permit-all chain — do **not** add a custom `SecurityFilterChain` bean for that case, it conflicts. This same `operaton.bpm.oauth2.identity-provider.enabled` flag also drives `JWTAuthenticationProvider`'s Keycloak-vs-Basic choice above — one source of truth for "is this deployment running with Keycloak login". See `backend/README.md`'s "`engine-rest` authentication modes" section for the full picture.
...
- `packages.jar` — the built Spring Boot jar; `packages.default` — a launcher script wrapping the jar with `temurin-jre-bin-21`; `packages.image` — a `dockerTools.streamLayeredImage` container (`datakurre/plone-operaton`).
- `devShells.default` — `jdk21`, `maven`, the `mvn2nix` CLI itself, plus `jq`, `entr`, `poetry`, `cachix`, `jfrog-cli`.
...
| Package | Purpose |
|---|---|
| `@bpmn-io/form-js-viewer` | Camunda Form JSON renderer (task/start forms) |
| `@bpmn-io/form-js` | Form modeler + playground (control panel) |
| `@bpmn-io/feel-editor`, `@bpmn-io/feelers-editor` | FEEL expression editors |
| `@bpmn-io/properties-panel` | Shared properties-panel framework |
| `bpmn-js` | BPMN 2.0 diagram viewer and modeler |
| `bpmn-js-properties-panel` | BPMN properties panel |
| `camunda-bpmn-moddle` | Camunda BPMN extension elements |
| `dmn-js` | DMN editor |
| `dmn-js-properties-panel` | DMN properties panel |
| `split.js` | Split-pane layout in the modeler control panel |
| `vite` (library mode, IIFE) | Module bundler (dev dependency) |
| `typescript` | Type-checking via `npm run typecheck` (dev dependency) |
...
There are 38 test modules under `backend/src/collective/bpmproxy/tests/`
(plus four Robot suites in `tests/robot/`). Rather than list them all here,
note the grouping -- the file names track the module they cover:

| Group | Files | Coverage |
|---|---|---|
| Install / profiles | `test_setup.py`, `test_upgrades.py`, `test_modeler_upgrades.py`, `test_renovation_demo.py` | Profiles, upgrade steps, demo profiles |
| Content types | `test_ct_bpm_proxy.py`, `test_ct_bpm_attachment.py`, `test_ct_bpm_attachments.py` | Dexterity CRUD |
| Views | `test_view_view.py`, `test_bpm_form_view.py`, `test_bpm_attachments_view.py`, `test_bpm_attachments_listing.py`, `test_bpm_attachments_orphans_view.py`, `test_controlpanel.py` | Rendering, submit, traversal |
| Client / utils | `test_client.py`, `test_client_more.py`, `test_utils.py`, `test_utils_more.py` | REST facade, variable inference, form prep/validation |
| REST services | `test_services_deploy.py`, `test_services_deployments.py`, `test_services_security.py` | `@bpmproxy-deploy`, `@bpmproxy-deployments` |
| Portlets / actions | `test_portlet_tasks.py`, `test_portlet_signal.py`, `test_action_signal.py` | Task list, signal dispatch |
| Adapters / viewlets | `test_security.py`, `test_patterns.py`, `test_substitutions.py`, `test_viewlets.py`, `test_viewlet_bpm_attachments_viewlet.py` | Local roles, widgets, substitutions |
| Subscribers / vocabularies | `test_subscribers.py`, `test_tasks.py`, `test_vocab_available_process_definitions.py`, `test_vocabularies_more.py` | Auto-complete, vocabularies |
| Live engine | `test_live_engine.py` | Marked `operaton`; needs a running engine (`make test-live`) |
| Robot | `test_robot.py`, `robot/*.robot` | Excluded from pytest; needs `zope.testrunner` (known follow-up) |
...
- **GitHub Actions** (`.github/workflows/plone-package.yml`), triggered on `master`/`main`:
  - `lint` — ruff check + format check over `backend/src` and `scripts/`, via `uvx`.
  - `test` — Python 3.10–3.13 matrix, `uv sync --extra test --locked`, pytest.
  - `build` — `uv build` + `twine check`.
  - `frontend` — `npm ci`, `npm run typecheck`, `npm run build`, then
    `git diff --exit-code` over `browser/static` so a stale committed bundle
