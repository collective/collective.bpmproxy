...
one signal, one process definition -- so Cockpit only ever has to enter one
...
left information panel while keeping the audit panel visible, enables the time
heatmap, and remains there for five seconds.
...
once, right after Author's turn creates the first instance, and stay on that
instance's own auto-refreshing view for the rest of the run -- there is no
second or third process to re-enter. The parallel-review sub-process is the
visual payload: once Reviewer3 delegates, the diagram shows two concurrent
tokens sitting on **Submit review**, one per reviewer, until both branches
join.
...
| `review-process-pip.webm` | Full HD focus-flipping composite: Cockpit is the main view while nothing is happening, but flips to Plone-as-main (small Cockpit inset) for the span of each persona's turn, then flips back |
...
| `review-process-cockpit-completed.png` | Cockpit: the completed process instance opened in History with the time heatmap enabled |
...
40 / 0.4 = 100s of that -- the back half, including the final History/heatmap
