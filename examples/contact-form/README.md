...
- **Start Form (`contact-form-start.form`)**: Allows anonymous visitors or logged-in users to submit inquiries (`senderName`, `senderEmail`, `subject`, `message`).
- **Review Task (`contact-form-review.form`)**: Assigned to Plone `Administrators`. Displays the submitted message and provides three decisions:
  ...
  - **Delegate**: Assign the inquiry to a specific Plone username (`assignedUser`).
  - **Abandon**: Close the inquiry without sending a response.
